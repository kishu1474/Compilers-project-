...
public class My_code_2 {
		...
		for (int i = 0; i < expression.length(); i++) {
			...
			if (expression.charAt(0) == '+' || expression.charAt(0) == '-' || expression.charAt(0) == '('
				...
				if (expression.charAt(i) == '+') {
					...
					final_exp += expression(expression.substring(0, last_plus));
					final_term += term(expression.substring(last_plus + 1));
	...
	public static String term(String term) {
		...
		for (int i = 0; i < term.length(); i++) {
			if (term.charAt(i) == '*') {
				...
				if ((m_1.find() && m_1.group().equals(final_term)) && (m_2.find() && m_2.group().equals(final_fact))) {
	...
	public static String fact(String fact) {
		...
		if (!fact.contains("+") && !fact.contains("-") && !fact.contains("(") && !fact.contains(")")) {
				...
				if (literal.charAt(i) == '0') {
					if (literal.length() > 1) {
						if (literal.charAt(1) != '0') {
							...
							output_1 = non_zero_digit(literal.charAt(1));
						} else {
							output_1 = false;
							return "";
						}
